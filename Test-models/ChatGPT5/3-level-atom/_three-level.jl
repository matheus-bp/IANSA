...
ν12 = (E2 - E1)/h
λ12 = c/ν12
A21 = 6.25e8               # s^-1 (toy; large to make line strong)
...
for iter = 1:maxiter
    ...
    Jν = zeros(nλ, Nz)
    for (jν, νval) in enumerate(ν)
        # Build monochromatic opacity including profile
        φs = [phi_nu(νval, ν12, Δν_D) for _ in 1:Nz]  # same φ at each depth for simplicity (no velocity gradient)
        κν = κ_total .* (1.0 .+ 0.0) .+ (n1 .* B12 .- n2 .* B21) .* (h*νval/(4*pi)) .* φs # rough
        # ensure positivity
        κν = abs.(κν) .+ 1e-30
        # monochromatic source S = j/κ
        jν = (h*νval/(4*pi)) .* n2 .* A21 .* φs  # emissivity from spontaneous emission only
        S = jν ./ κν
        ...
        # mean intensity J(ν) = 0.5 * ∫_{-1}^{1} I dμ ≈ 0.5 * sum(w_i * I(μ_i))
        for iz in 1:Nz
            Jν[jν, iz] = 0.5 * sum(wmu .* Iμ[:, iz])
        end
    end
...
for (jν, νval) in enumerate(ν)
    ...
    κν = abs.(κν) .+ 1e-30
    jν = (h*νval/(4*pi)) .* n2 .* A21 .* φs
    S = jν ./ κν
    Iμ = zeros(length(mus), Nz)
    for (ia, μ) in enumerate(mus)
        Iμ[ia, :] = formal_solver_short_characteristics(S, κν, τ, μ)
    end
    # emergent flux at top: Fν = 2π ∫_0^1 μ I(μ, τ=0) dμ
    I_at_surface = Iμ[:, 1]
    Fnu[jν] = 2*pi * sum(wmu .* max.(0.0, mus) .* I_at_surface)
end
